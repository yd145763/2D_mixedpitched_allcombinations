...
for s in Small_Pitch:
    for sc, bc in zip(Small_Count, Big_Count):
        ...
        #import data as pdf
        url = 'https://raw.githubusercontent.com/yd145763/2D_mixedpitched_allcombinations/main/big1200-'+str(sc)+'_small'+str(s)+'-'+str(bc)+'.csv'
        df = pd.read_csv(url)

        print('big1200-'+str(sc)+'_small'+str(s)+'-'+str(bc))
        ...
        grating_length = ((1200*bc) + (s*sc))/1000
        ...
        plt.title('big1200-'+str(sc)+'_small'+str(s)+'-'+str(bc), fontweight = 'bold')
        plt.show()
        plt.close()
        ...
        second_highest_e_field_FWHM_index = pd.Index(max_e_field_peak_FWHM[0]).get_loc(second_highest_e_field_FWHM)
